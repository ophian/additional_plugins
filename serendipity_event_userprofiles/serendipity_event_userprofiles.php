...
';
                ...
                case 'entries_header':
                    if (!empty($serendipity['GET']['viewAuthor']) && false === strpos($serendipity['GET']['viewAuthor'], ';')) {
                        ...
                        serendipity_plugin_api::hook_event('frontend_display', $entry, array('from' => 'serendipity_event_userprofiles:entries_header'));
...
<?php
                ...
                case 'backend_sidebar_users':// Up from Styx 3.6
                    if (version_compare($serendipity['version'], '3.6', '>=')) {
?>
                        <li class="list-flex"><div class="flex-column-1"><a href="?serendipity[adminModule]=event_display&amp;serendipity[adminAction]=profiles"><?php echo PLUGIN_EVENT_USERPROFILES_TITLE ?></a></div></li>
<?php
                    }
                    break;
                ...
                if ($serendipity['dbType'] == 'mysqli') {
                    ...
                    if (stristr(strtolower($serendipity['db_server_info']), 'mariadb')) {
                        ...
                        if (version_compare($db_version_match[0], '5.7.7', '>=')) {
                            $q = "CREATE UNIQUE INDEX userprofile_uidx ON {$serendipity['dbPrefix']}profiles (authorid, property);"; // Oracle Mysql/InnoDB max key up to 3072 bytes
                        } else {
