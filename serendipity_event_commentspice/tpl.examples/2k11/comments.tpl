...
{if $comment.avatar}{$comment.avatar}{/if}
    <h4>{if $comment.url}<a href="{$comment.url}" rel="nofollow">{/if}{$comment.author|@default:$CONST.ANONYMOUS}{if $comment.url}</a>{/if} {if $comment.spice_twitter_name}<a href="{$comment.spice_twitter_url}" target="_blank"{if $comment.spice_twitter_nofollow} rel="nofollow"{/if}>{$comment.spice_twitter_icon_html}</a>{/if} {$CONST.ON} <time datetime="{$comment.timestamp|@serendipity_html5time}" pubdate>{$comment.timestamp|@formatTime:$template_option.date_format}</time>:</h4>
