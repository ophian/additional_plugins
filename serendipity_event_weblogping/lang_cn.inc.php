...
@define('PLUGIN_EVENT_WEBLOGPING_CUSTOM_BLAHBLA', '可在此处定义多个其他请求服务(ping services)，不同请求服务之间使用逗号分割开来。应按照如下格式输入相关内容：服务器域名/路径 (host.domain/path)。如果在服务器域名前面有一个星号(*)的话，那么扩展的XML-RPC选项也将会被发送到该服务器（仅在该服务器支持此功能的情况下才有效 You have to submit this form several times, until the new entries are placed into the upper list.');
