...
@define('PLUGIN_EVENT_FREETAG_MANAGE_TAGS_NONE', 'Es existieren noch keine Einträge mit dazugehörigen Tags!');
...
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS_OPT', 'Auto-Tag Vorschläge anzeigen');
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS_OPT_DESC', 'Auf der TF-IDF Methode basierende Auto-Tag Vorschläge erhalten? Widersprechen Sie, wenn Sie generell andere Spracheinstellungen und Sprachen für Einträge als (augenblicklich) Deutsch oder Englisch nutzen.');
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS_LLM', 'Auto-Tag Vorschläge per (KI) LLM API');
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS_LLM_DESC', 'Experimentelle Ergänzung um Auto-Tag Vorschläge von einer "Künstlichen Intelligenz" zu erhalten, in dem der Text automatisiert an ein Large Language Model (LLM) gesendet wird. Schreibe ‹ none ›, oder leer lassen für NO-USE. Setzen Sie Ihren LLM API Key zu der LLM der Wahl (derzeit unterstützt: Anthropic (claude-sonnet-5), Gemini (gemini-3.1-flash-lite), OpenAI (gpt-5.6-luna) und Ollama (gemma4) local unter "http://localhost:11434"). Diese experimentelle Funktion zur Verwendung von LLMs für automatische Tags ist noch nie LIVE getestet worden. Falls Sie Probleme feststellen, diese Funktion erweitern möchten, sich das Modell oder die API-URLs dieser LLMs ändern, geben Sie bitte Feedback.');
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS', 'Auto-Tag Vorschläge');
@define('PLUGIN_EVENT_FREETAG_AUTOTAGS_INFO', 'Auto-Tag Vorschläge sind augenblicklich nur für [en/de] Sprachen verfügbar. Sie funktionieren vor allem aufgrund ihrer unterschiedlichen sprachlichen Anforderungen anders: Im Deutschen werden Substantive anhand der Großschreibung abgeglichen, während im Englischen kuratierte Stoppwortlisten zum Einsatz kommen. Wenn Sie also Tag Vorschläge erhalten die nicht unbedingt vorgeschlagen werden sollten, stellen Sie diese im freetag autotag discussion thread #71 auf GitHub zur Diskussion, um später in künftigen Plugin updates enthalten zu sein.
