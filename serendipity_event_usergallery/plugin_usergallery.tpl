...
<section id="page_{$staticpage_pagetitle|makeFilename}" class="serendipity_staticpage page serendipity_event_usergallery">
    ...
    {if $plugin_usergallery_dir_list == 'yes'}
        ...
        {else}
            ...
            <li class="open">{*<a href="{$plugin_usergallery_url}">*}<strong>{$head_title|default:$plugin_usergallery_title}</strong> ({$plugin_usergallery_maindir_filecount} {if $plugin_usergallery_maindir_filecount == 1}{$CONST.IMAGE}{else}{$CONST.PLUGIN_EVENT_USERGALLERY_IMAGES}{/if}){* </a> *}</li>
            {/if}
        {/if}
    ...
    {/if}
        ...
        <!-- end album list -->
    {if $plugin_usergallery_pagination}{* $plugin_usergallery_limit_directory != '' *}
    ...
    {foreach $plugin_usergallery_images AS $image}
            ...
            {if $plugin_usergallery_image_gallery|default:false}<div class="xserendipity_gallery_thumbs col"{* style="width: {$plugin_usergallery_colwidth}%;"*}>{/if}
        {if $image.isimage}{* $image.dimensions_width} < {$plugin_usergallery_fixed_width} - {$image.dimensions_height} < {$plugin_usergallery_fixed_width  AND ($image.dimensions_width < $plugin_usergallery_fixed_width OR $image.dimensions_height < $plugin_usergallery_fixed_width)*}
            ...
            {else}
                <!-- show thumb -->
                {if $plugin_usergallery_use_reltype}
        ...
        {/if}
            ...
            {if $plugin_usergallery_image_gallery|default:false}</div>{/if}
    {/foreach}
        </div><!-- row end -->
        <!-- end images -->
