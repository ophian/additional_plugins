                ...
                <div id="feed_header_{$feed@iteration}" class="feed_header">
                    <div class="feed_title">
                        <label for="multi-select-feed-{$feed@iteration}" class="num">
                            ...
                            {if $show_feedauthor && $feed.comments === null}<span class="feed_headline feed_author"><b>{$CONST.AUTHOR}:</b> {$feed.author|truncate:30:"&hellip;"} {$feed.action_author|default:''} </span>{/if}
                            <span class="feed_headline feed_timestamp">[{$feed.timestamp|formatTime:'%Y-%m-%d'|default:''}]</span><br>
                            <span><a href="{$feed.link}" title="{$feed.title}">{$feed.title|truncate:58:"&hellip;"}</a></span>
                        </label>
                        <time datetime="{$feed.pubDate|default:''}" pubdate></time>
                    </div>
                </div>
            {if $show_feedcontent}
                <details class="feed_data odd close">
                    ...
                    <div class="feed_boxed">
                        ...
                        {if $show_feedauthor && $feed.comments !== null}
                            <li class="mod_author"><b>{$CONST.AUTHOR}:</b> {$feed.author|truncate:30:"&hellip;"} {$feed.action_author|default:''} </li>
                        {/if}
