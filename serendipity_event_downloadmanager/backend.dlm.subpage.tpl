...
{* this info text constant contains linebreaks while there are 3 lines of information to understand *}
            <span class="msg_notice"><span class="icon-info-circled" aria-hidden="true"></span> {$CONST.PLUGIN_DOWNLOADMANAGER_INCOMINGTABLE_BLAHBLAH|sprintf:$dlmtfp.ftppath:$dlmgbl[0].cat.payload}
            </span>
...
{if $dlmtfp.ddiv === false}
    <script type="text/javascript">document.getElementById('dlm2').style.display = 'none';</script>
{/if}
{/if}
...
{if $dlmtsl.ddiv === false}
    <script type="text/javascript">document.getElementById('dlm3').style.display = 'none';</script>
{/if}
{/if}
...
{/if}
{foreach $dlmhcs.catlist[cat].imgname AS $s}
                        <img src="{$dlmgbl.httppath}img/{$s}.gif" alt="tree">
{/foreach}
{if $dlmhcs.catlist[cat].cat.hidden != 1}
