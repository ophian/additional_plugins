{* dlmanager.filelist.tpl last modified 2023-12-02 *}
...
{section name="file" loop=$fltable}
            <tr class="dlm_file {cycle name="cycle2" values="odd,even"}">
                <td class="dlm_filename">
                    <a href="{$fltable[file].info.iconurl}" class="dlm_fileicon"><img src="{$fltable[file].info.iconfile}" width="{$fltable[file].info.iconwidth}" height="{$fltable[file].info.iconheight}" alt="{$fltable[file].info.icontype}" title="{$fltable[file].info.icontype}"></a>
{if $fltable[file].is.showfilename}
                    <a href="{$fltable[file].info.nameurl}" class="dlm_filename" title="{$fltable[file].file.realfilename}">{$fltable[file].file.realfilename}</a>
{/if}
{if !empty($fltable[file].is.showdesc_inlist) && $fltable[file].info.file_desc}
                    <span class="dlm_filedesc">{$fltable[file].info.file_desc}</span>
{/if}
                </td>
