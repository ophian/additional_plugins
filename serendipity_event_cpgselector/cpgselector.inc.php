...
function cpg_end($db) {
    ...
    if (!empty($connparts[1])) {
        ...
        $serendipity['dbConn'] = mysqli_connect($connparts[0], $serendipity['dbUser'], $serendipity['dbPass'], $serendipity['dbName'], $connparts[1]);
    } else {
        // Connect with default ports
        $serendipity['dbConn'] = mysqli_connect($connparts[0], $serendipity['dbUser'], $serendipity['dbPass']);
    }
...
function cpg_start() {
    ...
    $db = mysqli_connect($CPG['server'], $CPG['user'], $CPG['password']);
    ...
    if (! @mysqli_select_db($db, $CPG['database']) ) {
...
function cpg_getAlbums() {
    ...
    while ($row = mysqli_fetch_array($result)) $albums[] = $row;
