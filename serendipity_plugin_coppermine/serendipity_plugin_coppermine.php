...
<?php
		...
		while ($row = mysql_fetch_array($result)) {
			...
			if ($albumlink){
				$albumresult = @mysqli_query($DB, "SELECT * FROM " . $prefix . "albums WHERE aid = '" . $albumid . "'");
				...
				while ($albumname = mysqli_fetch_array($albumresult)) {
...
<?php
	...
	function finished(&$db)
    {
		...
		mysqli_select_db($db, $serendipity['dbName']);
	}
