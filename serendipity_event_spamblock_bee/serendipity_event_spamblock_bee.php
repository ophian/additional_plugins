...
class serendipity_event_spamblock_bee extends serendipity_event
{
    ...
    function __construct($instance)
    {
        ...
        $this->useHoneyPot           = serendipity_db_bool($this->get_config('do_honeypot', 'true'));
        $this->hiddenCaptchaHandle   = $this->get_config('do_hiddencaptcha', PLUGIN_EVENT_SPAMBLOCK_SWTCH_MODERATE);
        $this->useRegularExpressions = serendipity_db_bool($this->get_config('use_regexp', 'false'));
    }
        ...
        if (isset($hooks[$event])) {
            ...
            switch($event) {
                ...
                case 'frontend_footer':
                    ...
                    $eventData['GET']['id'] = $eventData['GET']['id'] ?? ($eventData['entry_id'] ?? ($serendipity['GET']['id'] ?? null));
                    if ((!empty($eventData['GET']['id']) && $serendipity['view'] == 'entry' && in_array($type, ['all', 'entry'])) || ($contactFormInstalled && $serendipity['view'] == 'plugin' && (isset($serendipity['viewtype']) && $serendipity['viewtype'] == '404_4') && in_array($type, ['all', 'ctfrm']))) {
                        $this->printJsExtras();
                    }
                    break;
    ...
    function checkComment(&$eventData, &$addData)
    {
        ...
        if ("NORMAL" == $addData['type']) { // only supported for normal comments
            ...
            if (PLUGIN_EVENT_SPAMBLOCK_SWTCH_OFF != $this->hiddenCaptchaHandle) {
                ...
                $correctAnswer['answer'] = strtolower((string) ($correctAnswer['answer'] ?? ''));
                $isCorrect               = false;
    ...
    function printCommentEditExtras(&$eventData, &$addData)
    {
        ...
        if ((isset($eventData['GET']['action']) && $eventData['GET']['action']=='admin') || (isset($serendipity['serendipityUserlevel']) && (int)$serendipity['serendipityUserlevel'] >= (int)USERLEVEL_ADMIN)) return;
...
<?php
        ...
        $beecss .= "/* serendipity_event_spamblock_bee end */\n";
        $eventData .= $beecss;
    }
