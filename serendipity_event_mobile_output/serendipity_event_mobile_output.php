...
@define('PLUGIN_EVENT_MOBILE_VERSION','2.0.1');
...
class serendipity_event_mobile_output extends serendipity_event
{
    ...
    function event_hook($event, &$bag, &$eventData, $addData = null)
    {
        ...
        // plugin enabled?
        if (!serendipity_db_bool($this->get_config('enable'. 'true')))
            return false;
        ...
        if (isset($hooks[$event])) {
            switch($event) {
                ...
                case 'frontend_configure':
                    ...
                    if (serendipity_db_bool($this->get_config('redirect', 'false')) &&  $this->get_config('redirect_url') != $_SERVER['HTTP_HOST'] && !strstr($_SERVER['REQUEST_URI'],'serendipity_admin.php')) {
                ...
                case 'entry_display':
                    ...
                    if (serendipity_db_bool($this->get_config('categories', 'true'))) {
                        $this->debugItems[] = 'Adding categories to footer navigation';
                        $this->assignCategories($serendipity);
                    }

                    $smallteaser = serendipity_db_bool($this->get_config('smallteaser', 'true'));
    ...
    function cleanupMobileHtml( &$textpart, &$serendipity )
    {
        ...
        // scale images?
        if (serendipity_db_bool($this->get_config('images', 'false')) && ($this->get_config('scale_image_width') >0 || serendipity_db_bool($this->get_config('wurfl', 'false')))) {
    ...
    {
        if (serendipity_db_bool($this->get_config('wurfl', 'false'))) {
    ...
    function generateSitemap(&$bag)
    {
        ...
        $pingback_url = serendipity_db_bool($this->get_config('sitemap_pingback_urls', 'false'));
