...
@define('PLUGIN_EVENT_MULTILINGUAL_PURGE_INFO_DESC', 'You came along, that this additional selection push can be used to create and manage a blog entry multilingually.
   <p><strong>Recap:</strong> To do this, write a blog entry in the actual blog language (example) "English" and save it as usual. When you call it again from
   ...
   saving the entry again as usual.</p>
   <p>The Header "success" messages that appear after saving are designed for the normal entry worklow, and tell you, that the entry has been saved,
   although you have pushed a force delete checking the checkbox before. Don\'t let that deter you.</p>
   <p>This language entry should now be deleted from the database entryproperties table, but it is still being displayed in the entry form
   after saving because you are still in the Spanish language configuration setting. Another immediate SAVE would push the current content into again.</p>
   <p>Now, switch back to "German" (your first multilingual entry copy) or better jump to the backends entries list, and you’ll notice that
   the notification signal for the extended Spanish language entry "es" has disappeared.</p>'); // Translators, check the language this is written for and use equivalent example languages
@define('PLUGIN_SIDEBAR_MULTILINGUAL_JS_LANG_CHANGE_NOTIFICATION', 'ATTENTION: You previously selected the language "%s". If you select standard "Default", no active language will be loaded. "This is the End, my friend". Care for your work, since even now or after changing to default via the button, a rashly direct SAVE would overwrite your origin entry.');
