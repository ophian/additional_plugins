...
<div class="serendipity_entry">
	...
	<table width="100%" border="0" cellspacing="2" cellpadding="2">
		<form name="postform" action="{$ACTUALURL}" method="POST"{if $uploadform} enctype="multipart/form-data"{/if}>
			...
			<input type="hidden" name="serendipity[edit]" value="{$edit}" />
        ...
        <tr style="background-color: {$bgcolor2};">
			<td><textarea style="width: 95%;" rows="12" width="95%" name="serendipity[comment]">{$POST_MESSAGE}</textarea></td>
		</tr>
        
        {if $announcement}
            <tr style="background-color: {$bgcolor2};">
                <td width="100">{$CONST.PLUGIN_FORUM_ANNOUNCEMENT}</td>
                <td><input type="checkbox" name="serendipity[announcement]" value="1"{$checked} /></td>
            </tr>
        {/if}
