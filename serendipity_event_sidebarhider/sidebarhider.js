...
function addSideBarHiders(sideBars) {
    for (i = 0; i < sideBars.length; i++) {
        ...
        for (j = 0; j < sideBar.length; j++) {
            ...
            // unset a possible floated style of title
            title.style.styleFloat = "unset";
            title.style.cssFloat = "unset";
...
function sideBarHideRun() {
    ...
    if (sideBarA) {
        _sideBars[1] = getSideBars(sideBarA);
    }

    var sideBarB = document.getElementById('serendipityRightSideBar');
    if (sideBarB) {
        _sideBars[2] = getSideBars(sideBarB);
    }
