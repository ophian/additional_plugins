...
@define('PLUGIN_EVENT_OUTDATE_TIMEOUT_EXPIRY_FIELD', 'Přídavné pole "Datum expirace"');
@define('PLUGIN_EVENT_OUTDATE_TIMEOUT_EXPIRY_FIELD_DESC', 'Pokud používáte přídavný modul "Rozšířené vlastnosti příspěvků", můžete definovat přídavné pole, do kterého zadáte datum, kdy příspěvku vyprší platnost. Datum musí mít formát RRRR-MM-DD. Tento plugin najde toto datum vypršení platnosti a nastaví příspěvek jako KONCEPT, takže zmizí ze zobrazení příspěvků. Zde zadejte název přídavného pole (například "DatumExpirace").');
