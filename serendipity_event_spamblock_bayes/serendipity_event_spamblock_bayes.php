...
class serendipity_event_spamblock_bayes extends serendipity_event
{
    ...
    function classify($comment = '', $type)
    {
        ...
        # get saved token values from database
...
';
    ...
    function displayMenu($subpage=0)
    {
        ...
        echo $this->smarty_show('admin/bayesNavigation.tpl',
            array(
                'css' => $css,
    ...
    function smarty_show($template, $data = null)
    {
        ...
        if (!empty($data)) $serendipity['smarty']->assign($data);

        $tfile = serendipity_getTemplateFile($template, 'serendipityPath');
        if (!$tfile || $tfile == $template) {
    ...
    function showRecyclerMenu($commentpage)
    {
        ...
        echo $this->smarty_show('admin/bayesRecyclermenu.tpl',
            array(
                'comments' => $comments,
    ...
    function showAnalysisMenu($commentpage=0)
    {
        if (isset($this->get['comments'])) {
            ...
            echo $this->smarty_show('admin/bayesAnalysismenu.tpl',
                array(
                    'comments' => $comments,
                    'commentpage' => $commentpage,
                    'path' => $this->path
    ...
    function showImportMenu()
    {
        ...
        echo $this->smarty_show('admin/bayesImportmenu.tpl',
            array(
                'trojaRegistered' => serendipity_db_bool($this->get_config('troja_registered', 'false')) === true
            )
        );
    }
    ...
    function showAnalysis($comment_id)
    {
        ...
        echo $this->smarty_show('admin/bayesAnalysis.tpl',
            array(
                'comments' => $comments,
                'types' => array_values($this->type)
            )
    ...
    /**
     * For email-notification and classification. Learn a SPAM or HAM and delete or approve.
     */
    function learnAction($id, $category, $action, $entry_id)
    {
        ...
        if ($action == 'delete') {
            ...
            serendipity_approveComment($id, $entry_id, true, false, $token);
        }
    }
    ...
    function importDatabase($importDatabase)
    {
        ...
        if ($this->get_config('dbversion', 2) == 3
                && ($serendipity['dbType'] == 'mysql' || $serendipity['dbType'] == 'mysqli')) {
