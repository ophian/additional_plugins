...
#@define('PLUGIN_CATEGORYTEMPLATES_DESC', 'Тази приставка Ви позволява да сменяте шаблона и някои други свойства в зависимост от избраната катергория');// translate new, see en
@define('PLUGIN_CATEGORYTEMPLATES_SELECT', 'Моля въведете име на папката на шаблона, който искате да използвате за тази категория. Относителните имена на папките започват под templates/ . Така например можете да използвате "blue" или "kubrick". Можете да въведете и име на под-папка, ако сте записали под-папката в template папката. В такъв случай можете да запишете например "blue/category1" или "blue/category2". Recommended use is to add a category own "Engine:" template.');
...
@define('PLUGIN_CATEGORYTEMPLATES_CATPRECEDENCE_DESC', 'Когато дадена статия е асоциирана с повече от една категории, този списък определя категорията, чийто шаблон ще се използва при визуализация на статията. Най-горната категория се взема предвид първа. To be able to save and activate your order changes for existing categorytemplates, you must first activate their checkbox(es) here.');
@define('PLUGIN_CATEGORYTEMPLATES_NO_CUSTOMIZED_CATEGORIES', 'Все още няма категории със собствени шаблони. In this case let this checkbox unchecked!');
