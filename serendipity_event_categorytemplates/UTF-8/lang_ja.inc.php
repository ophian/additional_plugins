...
#@define('PLUGIN_CATEGORYTEMPLATES_DESC', 'このプラグインは、選択されたカテゴリに依存するテンプレートと他のいくつかのプロパティを変更することを可能にします。');// translate new, see en
@define('PLUGIN_CATEGORYTEMPLATES_SELECT', 'このカテゴリで使用したいテンプレートのディレクトリ名を入力してください。相対ディレクトリ名は templates/ 構造で始まります。従って例えば「blue」や「kubrick」を使うことができます。You can also enter a subdirectory name, if you saved a subdirectory within your template directory as if it were a template on its own. Then you can enter i.e. "blue/category1" or "blue/category2". Recommended use is to add a category own "Engine:" template.');
