{*
 plugin_guestbook_backend_dbc.tpl v.3.65 2023-12-01 Ian
*}
...
{include file='./plugin_guestbook_backend_header.tpl'}
    <div class="gb_head">
{if !empty($gb_view)}
        <h2>{$CONST.PLUGIN_GUESTBOOK_ADMIN_VIEW}</h2> {$CONST.PLUGIN_GUESTBOOK_ADMIN_VIEW_DESC}
{/if}
{if !empty($gb_app)}
        <h2>{$CONST.PLUGIN_GUESTBOOK_ADMIN_APP}</h2> {$CONST.PLUGIN_GUESTBOOK_ADMIN_APP_DESC}
{/if}
{if !empty($gb_add)}
        <h2>{$CONST.PLUGIN_GUESTBOOK_ADMIN_ADD}</h2>
{/if}
{if !empty($gb_db)}
        <h2>{$CONST.PLUGIN_GUESTBOOK_ADMIN_DBC}</h2>
{/if}
    </div>
...
{/if}
{if isset($is_guestbook_admin_backup)}
...
{/if}
{/if}
{if !empty($is_guestbook_admin_insert)}
...
{/if}
{/if}
{if !empty($is_guestbook_admin_insfile_msg) || !empty($is_guestbook_admin_delfile_msg)}
        <div class="msg_success"><span class="icon-ok"></span> {$plugin_gb_dbc_message}!</div>
{/if}
{if !empty($is_guestbook_admin_dbempty)}
...
{/if}
{if !empty($is_plugin_gb_questionaire)}
