...
{
    public $title = PLUGIN_EVENT_SITEMAP_TITLE;
    ...
    function should_add($type)
    {
        if (serendipity_db_bool($this->get_config('avoid_noindex', 'true'))) {
            ...
            if (in_array($type, ['sm_categories', 'sm_authors', 'sm_archives', 'sm_tags'])) {
                return false;
            }
        }
    ...
    function add_authors(&$sitemap_xml)
    {
        ...
        $authors = serendipity_db_query(
                "SELECT authors.authorid AS id,
                        realname,
                        username,
                        email,
                        MAX(entries.last_modified) AS time
                   FROM {$serendipity['dbPrefix']}authors authors,
                        {$serendipity['dbPrefix']}entries entries
    ...
    function write_sitemap(&$eventData, $basefilename = 'sitemap.xml', $gnewsmode = false)
    {
        ...
        $pingback_url = $this->get_config('url', false); // this false isn't the strings default, but I think false is meant if nothing is set

        if (!$pingback_url) return;
    ...
    function send_ping($loc)
    {
        ...
        $ping = serendipity_request_url($loc);
        if (!isset($serendipity['last_http_request']) || $serendipity['last_http_request']['responseCode'] != '200') {
            throw new Exception("Something wrong with responseCode: {$serendipity['last_http_request']['responseCode']}? $ping");
            return false;
        } else {
            return true;
        }
